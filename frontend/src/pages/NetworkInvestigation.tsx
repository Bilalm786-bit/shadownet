...
import { HiOutlineSearch, HiOutlineShieldCheck, HiOutlineExclamation, HiOutlineChevronDown, HiOutlineChevronUp, HiOutlineGlobeAlt } from 'react-icons/hi';
    ...
    <div className="fade-in">
      <div className="section-header">
        <h2 className="section-title" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 28 }}>🌐</span> Network Investigation
        </h2>
      </div>
      <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20, maxWidth: 600 }}>
        Investigate any IP address or domain. ShadowNet runs VirusTotal, Censys, DNS recon, port scanning,
        SSL analysis, WHOIS, Shodan, geolocation, and AI-powered threat assessment.
      </p>
          ...
          <div className="invest-search-bar">
            <div style={{ flex: 1, position: 'relative' }}>
              ...
              <input className="input" style={{ paddingLeft: 48, fontSize: 15, height: 52 }} value={target}
                onChange={e => setTarget(e.target.value)} placeholder="Enter IP address or domain (e.g. 8.8.8.8, example.com)..." />
            </div>
            <button className="btn btn-primary btn-lg" type="submit" disabled={loading || !target.trim()}>
              {loading ? '⏳ Scanning...' : '🛡️ Investigate Network'}
            </button>
          </div>
        </form>
      </div>
      ...
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: 60 }}>
          ...
          <div style={{ fontSize: 16, color: 'var(--cyan)', marginBottom: 8, fontWeight: 600 }}>Scanning network target...</div>
          <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Running VirusTotal, Censys, DNS, port scan, SSL, WHOIS, Shodan, and AI analysis.</div>
          <div className="progress-bar" style={{ marginTop: 20, maxWidth: 400, margin: '20px auto 0' }}>
            <div className="progress-fill" style={{ width: '55%', animation: 'shimmer 2s infinite' }} />
          </div>
        </div>
      )}
      ...
      {result && !loading && (
        <div className="slide-up">
          <div className="grid-4 stagger" style={{ marginBottom: 20 }}>
            <div className="card stat-card slide-up"><div className="stat-icon" style={{ background: 'var(--cyan-dim)', color: 'var(--cyan)' }}><HiOutlineGlobeAlt /></div><div><div className="stat-value">{result.category || 'network'}</div><div className="stat-label">Category</div></div></div>
          ...
          <div className="card" style={{ marginBottom: 20, padding: 20, borderLeft: '3px solid var(--cyan)' }}>
            <HiOutlineShieldCheck style={{ color: 'var(--cyan)', fontSize: 20, marginRight: 8, verticalAlign: 'middle' }} />
