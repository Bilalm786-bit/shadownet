    ...
    <div className="fade-in">
      <div className="section-header">
        <div>
          <h2 className="section-title" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 28 }}>🔗</span> Website Intelligence
          </h2>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 6, maxWidth: 720 }}>
            Full website assessment: tech fingerprint, SSL, DNS, subdomains, Wayback, crawler,
            content discovery, JS endpoint mining, CMS enum, plus defensive vulnerability checks
            (security headers, CORS, takeover, open redirect, SQLi traces).
          </p>
        </div>
      </div>
          ...
          <div className="invest-search-bar">
            <div style={{ flex: 1, position: 'relative' }}>
              ...
              <input className="input" style={{ paddingLeft: 48, fontSize: 15, height: 52 }}
                ...
                placeholder="URL or domain (e.g. example.com)..." />
            </div>
            <button className="btn btn-primary btn-lg" type="submit" disabled={loading || !target.trim()}>
              {loading ? '⏳ Investigating…' : '🔗 Investigate Website'}
            </button>
          </div>
        </form>
      </div>
      ...
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: 60 }}>
          <div className="pulse" style={{ fontSize: 60, marginBottom: 16 }}>🕸️</div>
          <div style={{ fontSize: 16, color: 'var(--orange)', fontWeight: 600 }}>Analyzing website...</div>
          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 8 }}>
            Tech fingerprint, SSL, DNS, subdomains, content discovery, JS endpoints, vuln checks — running in parallel.
          </div>
